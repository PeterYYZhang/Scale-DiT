...
def _get_sage_mask_search_dirs(model_config: Dict[str, Any]) -> List[Path]:
    configured_dir = model_config.get("sage_mask_dir", None)
    search_dirs: List[Path] = []

    if configured_dir:
        configured_path = Path(configured_dir).expanduser()
        if configured_path.is_absolute():
            search_dirs.append(configured_path)
        else:
            config_dir = _get_config_dir()
            if config_dir is not None:
                search_dirs.append((config_dir / configured_path).resolve())
            search_dirs.append((Path.cwd() / configured_path).resolve())

    # Fall back to common relative locations so callers do not need absolute paths.
    search_dirs.append((Path(__file__).resolve().parent / "attn_masks").resolve())
    search_dirs.append((Path.cwd() / "attn_masks").resolve())
...
def _load_sage_mask_thl_128x64(
    ...
    *,
    ...
    # Precomputed mask filenames keyed by image size (H, W)
    size_to_filename = {
        ...
        (8192, 6144): "(8192, 6144)x(8192, 6144)_downsampled_128_test-window.pt",
        (4096, 2048): "(4096, 2048)x(4096, 2048)_downsampled_128_test-window.pt",
        (8192, 4096): "(8192, 4096)x(8192, 4096)_downsampled_128_test-window.pt",
        (4096, 10240): "(4096, 10240)x(4096, 10240)_downsampled_128_test-window.pt",
        (8192, 16384): "(8192, 16384)x(8192, 16384)_downsampled_128_test-window.pt",
    }
    mask_filename = size_to_filename.get((int(img_h), int(img_w)))
    if mask_filename is None:
        raise ValueError(f"Image size {img_h} not supported, please build the mask first.")

    mask_path = None
    searched_paths = []
    for base_dir in _get_sage_mask_search_dirs(model_config):
        candidate_path = base_dir / mask_filename
    ...
    if mask_path is None:
        raise FileNotFoundError(
            f"Block mask file does not exist: {mask_filename}. "
            f"Searched: {searched_paths}"
        )

    block_mask = torch.load(str(mask_path), map_location="cpu")
...
# now we don't need this, since we are using Sage blocksparse attention
try:
