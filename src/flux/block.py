...
def _load_blocksparse_mask_thl_128x64(
    ...
    *,
    ...
    try:
        ...
        mask_path_6k = "/scratch/yuyao/HiHi_T2I/src_new/flux/attn_masks/(6144, 6144)x(6144, 6144)_downsampled_128_test-window.pt"
        mask_path_4k = "/scratch/yuyao/HiHi_T2I/src_new/flux/attn_masks/(4096, 4096)x(4096, 4096)_downsampled_128_test-window.pt"
        mask_path_4k_6144 = "/scratch3/ck1_23/efficient-attn/src_new/flux/attn_masks/(4096, 6144)x(4096, 6144)_downsampled_128_test-window.pt"
        mask_path_1k = "/scratch/yuyao/HiHi_T2I/src_new/flux/attn_masks/(1024, 1024)x(1024, 1024)_downsampled_128_test-window.pt"
        mask_path_2k = "/scratch3/ck1_23/efficient-attn/src_new/flux/attn_masks/2048x2048_downsampled_128_test-window.pt"
        mask_path_3k = "/scratch3/ck1_23/efficient-attn/src_new/flux/attn_masks/3072x3072_downsampled_128_test-window.pt"

        size_to_path = {
            (4096, 4096): mask_path_4k,
            (4096, 6144): mask_path_4k_6144,
...
def _attn_forward_impl(
    ...
    if low_res_guidance is not None:
        if SAGE_BLOCKSPARSE_AVAILABLE and model_config.get("use_sage_blocksparse", False):
            ...
            if cached is not None and cached.device == query.device:
                block_map = cached
            else:
                block_map = _load_blocksparse_mask_thl_128x64(
                    model_config,
                    batch_size=batch_size,
                    heads=nheads,
                )
                _BLOCK_MASK_CACHE[cache_key] = block_map
            # Sage expects (B,H,N,D) or (N,H,D) with tensor_layout flag; we use HND here.
            block_map = block_map.to(query.device)
